...
%  Last Modified: Sat Apr 30 15:13:37 2016
%  Created By:
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
<<functionsAndDatasets, echo=FALSE, cache=TRUE, include = TRUE>>=
   ...
   xmean <- mean(summary_sample)
   (summary_sample - xmean)^2
   0.5*(sum((summary_sample - xmean)^2))
   sqrt( 0.5*(sum((summary_sample - xmean)^2)))
@
...
<<echo=FALSE, cache=FALSE, include = TRUE>>=
   ...
   b1 <- (37299 - 150*mean(GPA)*mean(salary))/(882126 - 150*3.25*3.25)

   mean(salary) - b1*mean(GPA) + b1*3.0

   (4374.1263 - 489.7940)/4374.1263
...
<<echo=FALSE, cache=FALSE, include = FALSE>>=
   ...
   wine_sum %>% group_by(spice) %>% summarize(mn = mean(taste))

   wine %>% group_by(fruit) %>% summarize(mn = mean(taste) - mean(wine$taste))

   wine %>% group_by(spice) %>% summarize(mn = mean(taste))
      summarize(mn = mean(taste) - mean(wine$taste))
   ...
   50 + c(-1, 1)*1.645*sqrt(.2)

   50 + c(-1, 1)*2.58*sqrt(.2)

   53 + c(-1, 1)*1.96*sqrt(.4)
