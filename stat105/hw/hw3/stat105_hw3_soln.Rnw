...
\begin{center}
<<hsito, fig.width=5, fig.height=5, out.width='.5\\maxwidth', echo=FALSE>>=
   nebr <- c(38, 27, 56, 49, 29, 35, 69, 45, 77, 27, 54, 42)
	mich <- c(27, 38, 21, 37, 23, 28, 23, 24, 34, 26, 20, 21)

   teams <- data.frame("score" = c(nebr,mich), "team" = factor(rep(c("Nebraska","Michigan"),each=12)))

   ggplot2::qplot(score,data=teams[which(teams$team == "Nebraska"),],binwidth = 10)
@
\end{center}
...
\ben
\item Calculate the first quartile, the median, and the third quartile using the quartile function $Q(p)$.  

   \textbf{Nebraksa}:

   \begin{align*}
      Q(.25) &= x_i + (n (.25) - 1 + .5) x_{i + 1}  \tag{since $n p + 0.5 = 3.50 $} \\
             &= x_3 + (12 (.25) - 1 + .5) x_{3 + 1}  \tag{since $p = 0.25$} \\
             &= 29 + (3 - 1 + .5) 35  \\
             &= 29 + (3 - 1 + .5) 35  \\
...
\begin{tabular}{ccc}\hline
   ...
   p & Nebraska Q(p) & Michigan Q(p) \\ \hline 
   .25 & $Q(.25) = x_3 + $
